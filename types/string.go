...
func buildRecvString(n *Recv, s *strings.Builder) {
	switch {
	case n.Type != nil:
		buildTypeString(n.Mod == "", n.Type, s)
		return
	case len(n.Parms) == 0:
		break
...
func buildTypeString(elideMod bool, n *Type, s *strings.Builder) {
	modPath := n.ModPath
	if elideMod {
		modPath = ""
	}
	...
	case len(n.Args) == 1:
		...
		if modPath != "" || !isOpType(n.Name) || isOpType(n.Args[0].Name) {
			s.WriteRune(' ')
		}
	case len(n.Args) > 1:
		s.WriteRune('(')
