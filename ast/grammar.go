...
func _DefAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	fail5:
		pos = pos3
		// Test
		if !_accept(parser, _TestAccepts, &pos, &perr) {
			goto fail6
		}
		goto ok0
	fail6:
...
func _DefFail(parser *_Parser, start, errPos int) (int, *peg.Fail) {
	...
	fail5:
		pos = pos3
		// Test
		if !_fail(parser, _TestFail, errPos, failure, &pos) {
			goto fail6
		}
		goto ok0
	fail6:
...
func _DefAction(parser *_Parser, start int) (int, *Def) {
	...
	fail5:
		...
		pos = pos3
		// Test
		if p, n := _TestAction(parser, pos); n == nil {
			goto fail6
		} else {
			node = *n
			pos = p
...
func _FunAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// _ f:(key:("func"/"Func") tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
	// _
	if !_accept(parser, __Accepts, &pos, &perr) {
		goto fail
	}
	// f:(key:("func"/"Func") tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
	{
		pos1 := pos
		// (key:("func"/"Func") tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
		// action
		// key:("func"/"Func") tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]"
		// key:("func"/"Func")
		{
	...
	// action
	{
		...
		// f:(key:("func"/"Func") tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
		{
			...
			// action
			{
				...
				// body:(_ "|" stmts:Stmts {…})?
				{
					...
					// (_ "|" stmts:Stmts {…})?
					{
						...
						// action
						{
							...
							*label4 = func(
								start, end int, key string, sig FunSig, stmts []Stmt, tps []Var) []Stmt {
								return []Stmt(stmts)
							}(
								start18, pos, label0, label2, label3, label1)
						}
						goto ok21
					fail17:
						label4 = nil
				...
				pos++
				label5 = func(
					start, end int, body *[]Stmt, key string, sig FunSig, stmts []Stmt, tps []Var) *Fun {
		...
		node = func(
			start, end int, body *[]Stmt, f *Fun, key string, sig FunSig, stmts []Stmt, tps []Var) Def {
			return Def(f)
		}(
			start0, pos, label4, label5, label0, label2, label3, label1)
	}
	parser.act[key] = node
	return pos, &node
fail:
...
func _TestAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	var labels [3]string
	use(labels)
	if dp, de, ok := _memo(parser, _Test, start); ok {
		return dp, de
	}
	pos, perr := start, -1
	// action
	// _ f:("test" _ "[" n:Ident _ "|" stmts:Stmts _ "]" {…})
	// _
	if !_accept(parser, __Accepts, &pos, &perr) {
		goto fail
	}
	// f:("test" _ "[" n:Ident _ "|" stmts:Stmts _ "]" {…})
	{
		pos1 := pos
	...
	return _memoize(parser, _Test, start, pos, perr)
fail:
	return _memoize(parser, _Test, start, -1, perr)
}

func _TestFail(parser *_Parser, start, errPos int) (int, *peg.Fail) {
	var labels [3]string
	use(labels)
	pos, failure := _failMemo(parser, _Test, start, errPos)
	if failure != nil {
		return pos, failure
	}
	failure = &peg.Fail{
	...
	key := _key{start: start, rule: _Test}
	// action
	// _ f:("test" _ "[" n:Ident _ "|" stmts:Stmts _ "]" {…})
	// _
	if !_fail(parser, __Fail, errPos, failure, &pos) {
...
func _TestAction(parser *_Parser, start int) (int, *Def) {
	var labels [3]string
	use(labels)
	var label0 Ident
	var label1 []Stmt
	var label2 *Fun
	dp := parser.deltaPos[start][_Test]
	if dp < 0 {
		return -1, nil
	}
	key := _key{start: start, rule: _Test}
	...
	// action
	{
		...
		node = func(
			start, end int, f *Fun, n Ident, stmts []Stmt) Def {
			return Def(f)
		}(
			start0, pos, label2, label0, label1)
	}
...
func _MethAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// _ m:(key:("meth"/"Meth") recv:Recv tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
	// _
	if !_accept(parser, __Accepts, &pos, &perr) {
		goto fail
	}
	// m:(key:("meth"/"Meth") recv:Recv tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
	{
		pos1 := pos
		// (key:("meth"/"Meth") recv:Recv tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
		// action
		// key:("meth"/"Meth") recv:Recv tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]"
		// key:("meth"/"Meth")
		{
		...
		// body:(_ "|" stmts:Stmts {…})?
		{
			...
			labels[5] = parser.text[pos13:pos]
		}
		// _
		if !_accept(parser, __Accepts, &pos, &perr) {
			goto fail
	...
	// action
	{
		...
		// m:(key:("meth"/"Meth") recv:Recv tps:TParms _ "[" sig:FunSig body:(_ "|" stmts:Stmts {…})? _ "]" {…})
		{
			...
			// action
			{
				...
				// body:(_ "|" stmts:Stmts {…})?
				{
					...
					// (_ "|" stmts:Stmts {…})?
					{
						...
						// action
						{
							...
							*label5 = func(
								start, end int, key string, recv Recv, sig FunSig, stmts []Stmt, tps []Var) []Stmt {
								return []Stmt(stmts)
							}(
								start19, pos, label0, label1, label3, label4, label2)
						}
						goto ok22
					fail18:
						label5 = nil
				...
				pos++
				label6 = func(
					start, end int, body *[]Stmt, key string, recv Recv, sig FunSig, stmts []Stmt, tps []Var) *Fun {
					if body != nil && stmts == nil {
						stmts = []Stmt{}
					}
					return &Fun{
			...
			labels[6] = parser.text[pos2:pos]
		}
		node = func(
			start, end int, body *[]Stmt, key string, m *Fun, recv Recv, sig FunSig, stmts []Stmt, tps []Var) Def {
			return Def(m)
		}(
			start0, pos, label5, label0, label6, label1, label3, label4, label2)
	}
	parser.act[key] = node
	return pos, &node
fail:
...
func _TParmsAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// tps:(n:TypeVar {…}/_ v:("_" !([_a-zA-Z0-9]) {…}) {…}/_ "(" p0:TParm ps:(_ "," p1:TParm {…})* (_ ",")? _ ")" {…})?
	{
		pos0 := pos
		// (n:TypeVar {…}/_ v:("_" !([_a-zA-Z0-9]) {…}) {…}/_ "(" p0:TParm ps:(_ "," p1:TParm {…})* (_ ",")? _ ")" {…})?
		{
			pos2 := pos
			// (n:TypeVar {…}/_ v:("_" !([_a-zA-Z0-9]) {…}) {…}/_ "(" p0:TParm ps:(_ "," p1:TParm {…})* (_ ",")? _ ")" {…})
			// n:TypeVar {…}/_ v:("_" !([_a-zA-Z0-9]) {…}) {…}/_ "(" p0:TParm ps:(_ "," p1:TParm {…})* (_ ",")? _ ")" {…}
			...
			fail10:
				...
				// p0:TParm
				{
					...
					labels[2] = parser.text[pos20:pos]
				}
				// ps:(_ "," p1:TParm {…})*
				{
					pos21 := pos
...
func _TParmsAction(parser *_Parser, start int) (int, *([]Var)) {
	...
	// action
	{
				...
				fail9:
					*label5 = node7
					...
					// action
					{
						...
						*label5 = func(
							start, end int, n Ident, v []Var) []Var {
							return []Var(v)
						}(
				...
				fail12:
					*label5 = node7
					...
					// action
					{
						...
						// ps:(_ "," p1:TParm {…})*
						{
							...
							for {
								...
								// action
								{
									...
									node29 = func(
										start, end int, n Ident, p0 Var, p1 Var, v []Var) Var {
										return Var(p1)
									}(
										start31, pos, label0, label2, label3, label1)
								}
						...
						pos++
						*label5 = func(
							start, end int, n Ident, p0 Var, p1 Var, ps []Var, v []Var) []Var {
							return []Var(append([]Var{p0}, ps...))
						}(
							start23, pos, label0, label2, label3, label4, label1)
					}
					goto ok5
				fail22:
					*label5 = node7
		...
		node = func(
			start, end int, n Ident, p0 Var, p1 Var, ps []Var, tps *[]Var, v []Var) []Var {
			if tps == nil {
				return ([]Var)(nil)
			}
			return ([]Var)(*tps)
		}(
			start0, pos, label0, label2, label3, label4, label5, label1)
	}
	parser.act[key] = node
	return pos, &node
}
...
func _TParmAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// n:(TypeVar/_ empty:("_" !([_a-zA-Z0-9]) {…}) {…}) t1:TypeName?
	// n:(TypeVar/_ empty:("_" !([_a-zA-Z0-9]) {…}) {…})
	{
		pos1 := pos
		// (TypeVar/_ empty:("_" !([_a-zA-Z0-9]) {…}) {…})
		// TypeVar/_ empty:("_" !([_a-zA-Z0-9]) {…}) {…}
		{
			pos5 := pos
			// TypeVar
		...
		// n:(TypeVar/_ empty:("_" !([_a-zA-Z0-9]) {…}) {…})
		{
			...
			fail7:
				...
				// action
				{
					...
					label1 = func(
						start, end int, empty Ident) Ident {
						return Ident(empty)
					}(
						start9, pos, label0)
				}
...
func _TNameAction(parser *_Parser, start int) (int, *tname) {
	...
	// action
	{
		...
		node = func(
			start, end int, mod *ModTag, n Ident) tname {
...
func _BinMsgAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// mod:ModName? n:Op a:(u:Unary {…}/Primary)
	// mod:ModName?
	{
		pos1 := pos
		// ModName?
		...
		// a:(u:Unary {…}/Primary)
		{
			...
			// u:Unary {…}/Primary
			{
				...
				// action
				{
					...
					label3 = func(
						start, end int, mod *ModTag, n Ident, u *Call) Expr {
						return Expr(u)
					}(
						start14, pos, label0, label1, label2)
				}
...
func _NaryMsgAction(parser *_Parser, start int) (int, *Msg) {
	...
	// action
	{
		...
		// as:(n:IdentC v:(b:Binary {…}/u:Unary {…}/Primary) {…})+
		{
			...
			// (n:IdentC v:(b:Binary {…}/u:Unary {…}/Primary) {…})+
			{
				...
				// action
				{
					...
					// v:(b:Binary {…}/u:Unary {…}/Primary)
					{
						...
						// b:Binary {…}/u:Unary {…}/Primary
						{
							...
							// action
							{
								...
								label4 = func(
									start, end int, b *Call, mod *ModTag, n Ident) Expr {
									return Expr(b)
								}(
						...
						fail20:
							...
							// action
							{
								...
								label4 = func(
									start, end int, b *Call, mod *ModTag, n Ident, u *Call) Expr {
									return Expr(u)
								}(
					...
					node10 = func(
						start, end int, b *Call, mod *ModTag, n Ident, u *Call, v Expr) arg {
						return arg{n, v}
					}(
		...
		node = func(
			start, end int, as []arg, b *Call, mod *ModTag, n Ident, u *Call, v Expr) Msg {
			var sel string
			var es []Expr
			for _, a := range as {
				sel += a.name.Text
...
func _PrimaryAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// mod:ModName? i:Ident {…}/Float/Int/Rune/s:String {…}/Ctor/Block/_ "(" e:Expr _ ")" {…}
	{
		pos3 := pos
		// action
		// mod:ModName? i:Ident
...
func _PrimaryAction(parser *_Parser, start int) (int, *Expr) {
	...
	// mod:ModName? i:Ident {…}/Float/Int/Rune/s:String {…}/Ctor/Block/_ "(" e:Expr _ ")" {…}
	{
		...
		// action
		{
			...
			node = func(
				start, end int, i Ident, mod *ModTag) Expr {
				i.Mod = mod
				return Expr(&i)
			}(
				start5, pos, label1, label0)
	...
	fail15:
		...
		// action
		{
			...
			node = func(
				start, end int, i Ident, mod *ModTag, s String) Expr {
				return Expr(&s)
			}(
				start17, pos, label1, label0, label2)
		}
		goto ok0
	fail16:
		node = node2
	...
	fail20:
		...
		// action
		{
			...
			pos++
			node = func(
				start, end int, e Expr, i Ident, mod *ModTag, s String) Expr {
				return Expr(e)
			}(
				start22, pos, label3, label1, label0, label2)
		}
		goto ok0
	fail21:
		node = node2
...
func _ModNameAction(parser *_Parser, start int) (int, *ModTag) {
	var labels [2]string
	use(labels)
	var label0 string
	var label1 ModTag
	...
	// action
	{
		...
		node = func(
			start, end int, text string, tok ModTag) ModTag {
			return ModTag(tok)
		}(
			start0, pos, label0, label1)
	}
