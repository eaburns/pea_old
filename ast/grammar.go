...
func _FileAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	// defss:Defs*
	{
		...
		for {
			pos3 := pos
			// Defs
			if !_accept(parser, _DefsAccepts, &pos, &perr) {
				goto fail5
			}
			continue
		fail5:
...
func _FileNode(parser *_Parser, start int) (int, *peg.Node) {
	...
	// defss:Defs*
	{
		...
		for {
			...
			pos3 := pos
			// Defs
			if !_node(parser, _DefsNode, node, &pos) {
				goto fail5
			}
			continue
		fail5:
...
func _FileFail(parser *_Parser, start, errPos int) (int, *peg.Fail) {
	...
	// defss:Defs*
	{
		...
		for {
			pos3 := pos
			// Defs
			if !_fail(parser, _DefsFail, errPos, failure, &pos) {
				goto fail5
			}
			continue
		fail5:
	...
	// action
	{
		...
		// defss:Defs*
		{
			...
			for {
				...
				var node5 []Def
				// Defs
				if p, n := _DefsAction(parser, pos); n == nil {
					goto fail6
				} else {
					node5 = *n
					pos = p
...
func _DefsAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	var labels [14]string
	use(labels)
	if dp, de, ok := _memo(parser, _Defs, start); ok {
		return dp, de
	}
	pos, perr := start, -1
	...
	fail10:
		...
		// s0:TypeSig
		{
			...
			labels[4] = parser.text[pos14:pos]
		}
		// t:Type
		{
			pos15 := pos
			...
			labels[5] = parser.text[pos15:pos]
		}
		goto ok0
	fail12:
		pos = pos3
		...
		// m:Fun
		{
			...
			labels[7] = parser.text[pos19:pos]
		}
		goto ok0
	fail16:
		pos = pos3
		// action
		// sig:TypeSig _ "(" ds3:(Type/Fun)+ _ ")"
		// sig:TypeSig
		{
			pos22 := pos
			// TypeSig
			...
			labels[8] = parser.text[pos22:pos]
		}
		// _
		if !_accept(parser, __Accepts, &pos, &perr) {
			goto fail20
		...
		// ds3:(Type/Fun)+
		{
			...
			labels[9] = parser.text[pos23:pos]
		}
		// _
		if !_accept(parser, __Accepts, &pos, &perr) {
			goto fail20
	...
	fail20:
		...
		// n0:ModName?
		{
			...
			labels[10] = parser.text[pos42:pos]
		}
		// _
		if !_accept(parser, __Accepts, &pos, &perr) {
			goto fail40
		...
		// ds1:Defs+
		{
			...
			labels[11] = parser.text[pos47:pos]
		}
		// _
		if !_accept(parser, __Accepts, &pos, &perr) {
			goto fail40
	...
	fail40:
		...
		// n1:ModName
		{
			...
			labels[12] = parser.text[pos54:pos]
		}
		// ds2:Defs
		{
			pos55 := pos
			...
			labels[13] = parser.text[pos55:pos]
		}
		goto ok0
	fail52:
		pos = pos3
	...
	return _memoize(parser, _Defs, start, pos, perr)
fail:
	return _memoize(parser, _Defs, start, -1, perr)
}

func _DefsNode(parser *_Parser, start int) (int, *peg.Node) {
	var labels [14]string
	use(labels)
	dp := parser.deltaPos[start][_Defs]
	if dp < 0 {
		return -1, nil
	}
		...
		// ds3:(Type/Fun)+
		{
			...
			for {
				...
				// (Type/Fun)
				{
					...
					sub := _sub(parser, pos037, pos, node.Kids[nkids36:])
					node.Kids = append(node.Kids[:nkids36], sub)
				}
				continue
			fail27:
	...
	fail20:
		...
		// ds1:Defs+
		{
			...
			for {
				...
				continue
			fail55:
				node.Kids = node.Kids[:nkids52]
				pos = pos53
				break
			}
			labels[11] = parser.text[pos51:pos]
		}
		// _
		if !_node(parser, __Node, node, &pos) {
			goto fail44
	...
	fail44:
		...
		// ds2:Defs
		{
			...
			labels[13] = parser.text[pos59:pos]
		}
		goto ok0
	fail56:
		node.Kids = node.Kids[:nkids1]
...
func _DefsFail(parser *_Parser, start, errPos int) (int, *peg.Fail) {
	var labels [14]string
	use(labels)
	pos, failure := _failMemo(parser, _Defs, start, errPos)
	if failure != nil {
		return pos, failure
	}
	failure = &peg.Fail{
...
func _DefsAction(parser *_Parser, start int) (int, *[]Def) {
	var labels [14]string
	use(labels)
	var label0 *Import
	var label1 Def
	...
	fail4:
		...
		// action
		{
			...
			node = func(
				start, end int, f Def, i *Import) []Def {
				return []Def{f}
			}(
	...
	fail13:
		...
		// action
		{
			...
			node = func(
				...
				start17, pos, label3, label1, label0, label4, label5, label2)
		}
		goto ok0
	fail16:
		node = node2
		...
		// action
		{
			...
			// s1:TypeSig
			{
				...
				labels[6] = parser.text[pos24:pos]
			}
			// m:Fun
			{
				pos25 := pos
	...
	fail21:
		...
		// action
		{
			...
			// sig:TypeSig
			{
				...
				labels[8] = parser.text[pos29:pos]
			}
			// _
			if p, n := __Action(parser, pos); n == nil {
				goto fail26
	...
	fail26:
		...
		// action
		{
			...
			// n0:ModName?
			{
				...
				labels[10] = parser.text[pos50:pos]
			}
			// _
			if p, n := __Action(parser, pos); n == nil {
				goto fail47
			...
			// ds1:Defs+
			{
				...
				labels[11] = parser.text[pos55:pos]
			}
			// _
			if p, n := __Action(parser, pos); n == nil {
				goto fail47
			...
			node = func(
				...
				start48, pos, label3, label11, label9, label1, label0, label7, label10, label4, label6, label8, label5, label2)
		}
		goto ok0
	fail47:
		node = node2
		...
		// action
		{
			...
			// n1:ModName
			{
				...
				labels[12] = parser.text[pos63:pos]
			}
			// ds2:Defs
			{
				pos64 := pos
			...
			node = func(
				...
				start61, pos, label3, label11, label13, label9, label1, label0, label7, label10, label12, label4, label6, label8, label5, label2)
		}
		goto ok0
	fail60:
		node = node2
...
func _ImportNode(parser *_Parser, start int) (int, *peg.Node) {
	var labels [1]string
	use(labels)
...
func _TypeNameNode(parser *_Parser, start int) (int, *peg.Node) {
	...
	fail18:
		...
		// n2:TypeName
		{
			...
			labels[6] = parser.text[pos32:pos]
		}
		// _
		if !_node(parser, __Node, node, &pos) {
			goto fail30
...
func _UnaryAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// r:(p:Primary {…}/n:ModPath {…}) ms:UnaryMsg+
	// r:(p:Primary {…}/n:ModPath {…})
	{
		pos1 := pos
		// (p:Primary {…}/n:ModPath {…})
		// p:Primary {…}/n:ModPath {…}
		...
		// r:(p:Primary {…}/n:ModPath {…})
		{
			...
			fail7:
				...
				// action
				{
					...
					label2 = func(
						start, end int, n ModPath, p Expr) Node {
						return Node(n)
					}(
...
func _BinaryAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// r:(u:Unary {…}/p:Primary {…}/n:ModPath {…}) m:BinMsg
	// r:(u:Unary {…}/p:Primary {…}/n:ModPath {…})
	{
		pos1 := pos
		// (u:Unary {…}/p:Primary {…}/n:ModPath {…})
		// u:Unary {…}/p:Primary {…}/n:ModPath {…}
		{
			pos5 := pos
			// action
	...
	// m:BinMsg
	{
		...
		labels[4] = parser.text[pos12:pos]
	}
...
func _BinaryAction(parser *_Parser, start int) (int, *Call) {
	...
	// action
	{
		...
		// r:(u:Unary {…}/p:Primary {…}/n:ModPath {…})
		{
			...
			fail7:
				...
				// action
				{
					...
					label3 = func(
						start, end int, p Expr, u Call) Node {
						return Node(p)
					}(
						start11, pos, label1, label0)
				}
			...
			fail10:
				...
				// action
				{
					...
					label3 = func(
						start, end int, n ModPath, p Expr, u Call) Node {
						return Node(n)
					}(
						start14, pos, label2, label1, label0)
				}
				goto ok3
			fail13:
				label3 = node5
		...
		node = func(
			...
			start0, pos, label4, label2, label1, label3, label0)
	}
	parser.act[key] = node
	return pos, &node
fail:
...
func _NaryAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// r:(b:Binary {…}/u:Unary {…}/p:Primary {…}/n:ModPath {…})? m:NaryMsg
	// r:(b:Binary {…}/u:Unary {…}/p:Primary {…}/n:ModPath {…})?
	{
		pos1 := pos
		// (b:Binary {…}/u:Unary {…}/p:Primary {…}/n:ModPath {…})?
		{
			pos3 := pos
			// (b:Binary {…}/u:Unary {…}/p:Primary {…}/n:ModPath {…})
			// b:Binary {…}/u:Unary {…}/p:Primary {…}/n:ModPath {…}
...
func _NaryAction(parser *_Parser, start int) (int, *Call) {
	...
	// action
	{
				...
				fail10:
					*label4 = node8
					...
					// action
					{
						...
						*label4 = func(
							start, end int, b Call, u Call) Node {
							return Node(u)
						}(
				...
				fail13:
					*label4 = node8
					...
					// action
					{
						...
						*label4 = func(
							start, end int, b Call, p Expr, u Call) Node {
							return Node(p)
						}(
							start17, pos, label0, label2, label1)
					}
					goto ok6
				fail16:
					*label4 = node8
					...
					// action
					{
						...
						*label4 = func(
							start, end int, b Call, n ModPath, p Expr, u Call) Node {
							return Node(n)
						}(
							start20, pos, label0, label3, label2, label1)
					}
					goto ok6
				fail19:
					*label4 = node8
		...
		node = func(
			start, end int, b Call, m Msg, n ModPath, p Expr, r *Node, u Call) Call {
			s := m.start
			var recv Node
			if r != nil {
				s, _ = (*r).loc()
			...
			start0, pos, label0, label5, label3, label2, label4, label1)
	}
	parser.act[key] = node
	return pos, &node
fail:
...
func _CtorAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// _ ("{" t:TypeName _ "|" as:(Struct/id:Ident {…}/a:Ary? {…}) _ "}")
	// _
	if !_accept(parser, __Accepts, &pos, &perr) {
		goto fail
	}
	// ("{" t:TypeName _ "|" as:(Struct/id:Ident {…}/a:Ary? {…}) _ "}")
	// "{" t:TypeName _ "|" as:(Struct/id:Ident {…}/a:Ary? {…}) _ "}"
	// "{"
	if len(parser.text[pos:]) < 1 || parser.text[pos:pos+1] != "{" {
		perr = _max(perr, pos)
	...
	pos++
	// as:(Struct/id:Ident {…}/a:Ary? {…})
	{
		pos3 := pos
		// (Struct/id:Ident {…}/a:Ary? {…})
		// Struct/id:Ident {…}/a:Ary? {…}
		{
			pos7 := pos
			// Struct
		...
		// as:(Struct/id:Ident {…}/a:Ary? {…})
		{
			...
			fail10:
				...
				// action
				{
					...
					label3 = func(
						start, end int, a *[]arg, id Ident, t TypeName) []arg {
						if a != nil {
							return *a
						}
						return []arg{}
...
func _StructAccepts(parser *_Parser, start int) (deltaPos, deltaErr int) {
	...
	pos, perr := start, -1
	// action
	// i0:IdentC e0:Expr args:(_ ";" i1:IdentC e1:Expr {…})* (_ ";")?
	// i0:IdentC
	{
		pos1 := pos
		// IdentC
...
func _StructAction(parser *_Parser, start int) (int, *[]arg) {
	var labels [5]string
	use(labels)
	var label0 Ident
	var label1 Expr
	...
	// action
	{
		...
		// args:(_ ";" i1:IdentC e1:Expr {…})*
		{
			...
			for {
				...
				// action
				{
					...
					node7 = func(
						start, end int, e0 Expr, e1 Expr, i0 Ident, i1 Ident) arg {
						return arg{name: i1, val: e1}
					}(
						start9, pos, label1, label3, label0, label2)
				}
				label4 = append(label4, node7)
				continue
			fail8:
				pos = pos6
				break
			}
			labels[4] = parser.text[pos4:pos]
		}
		// (_ ";")?
		{
			pos14 := pos
		...
		node = func(
			start, end int, args []arg, e0 Expr, e1 Expr, i0 Ident, i1 Ident) []arg {
			return []arg(append([]arg{{name: i0, val: e0}}, args...))
		}(
			start0, pos, label4, label1, label3, label0, label2)
	}
	parser.act[key] = node
	return pos, &node
fail:
...
func _AryAction(parser *_Parser, start int) (int, *[]arg) {
	var labels [3]string
	use(labels)
	var label0 Expr
	var label1 Expr
	...
	// action
	{
		...
		// es:(_ ";" e:Expr {…})*
		{
			...
			for {
				...
				// action
				{
					...
					node6 = func(
						start, end int, e Expr, e0 Expr) arg {
						return arg{val: e}
					}(
						start8, pos, label1, label0)
				}
		...
		node = func(
			start, end int, e Expr, e0 Expr, es []arg) []arg {
		...
		// ("[" ((ps:(n:CIdent t:TypeName? {…})+ _ "|")? ss:Stmts) _ "]" {…})
		{
			...
			// ((ps:(n:CIdent t:TypeName? {…})+ _ "|")? ss:Stmts)
			{
							...
							for {
								...
								// (n:CIdent t:TypeName? {…})
								{
									...
									// n:CIdent
									{
										...
										labels[0] = parser.text[pos31:pos]
									}
									// t:TypeName?
									{
										pos32 := pos
										...
										labels[1] = parser.text[pos32:pos]
									}
									sub := _sub(parser, pos029, pos, node.Kids[nkids28:])
									node.Kids = append(node.Kids[:nkids28], sub)
								}
				...
				// ss:Stmts
				{
					...
					labels[3] = parser.text[pos38:pos]
				}
				sub := _sub(parser, pos06, pos, node.Kids[nkids5:])
				node.Kids = append(node.Kids[:nkids5], sub)
			}
